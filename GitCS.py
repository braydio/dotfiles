...
# Display all available Git command categories
def display_categories():
    print(Fore.CYAN + "\n# ---------------------------------")
    print(Fore.CYAN + "\nAvailable Git Command Categories:\n")
    for key, category in categories.items():
        print(Fore.YELLOW + f"{key}: {category['category_name']}")
    print(Fore.CYAN + "\nEnter the number of a category to list its commands or type '0' or 'exit' to quit.")

# Display commands within a selected category
def display_commands(category_key):
    category = categories[category_key]
    print(Fore.CYAN + f"\n# ---------------------------------")
    print(Fore.GREEN + f"\nCommands in {category['category_name']}:\n")

    # Enumerate through commands with reset numbering for each category
    for idx, (cmd_key, cmd) in enumerate(category['commands'].items(), start=1):
        print(Fore.YELLOW + f"{idx}: {cmd['name']}")
    ...
    try:
        ...
        if category_key in categories:
            while True:
                ...
                command_input = input(Fore.CYAN + "Select a command number: ").strip()
                ...
                try:
                    ...
                    # Verify the command index is valid
                    if 1 <= command_idx <= len(command_list):
                        cmd = command_list[command_idx - 1]
                        print(Fore.CYAN + "\n# ---------------------------------")
                        print(Fore.GREEN + f"\nCommand: {cmd['command']}\nDetails: {cmd['details']}\n")
                        input(Fore.CYAN + "Press Enter to go back to the commands list.")
                    else:
